...
**Problem:** Asked to click through the app as a customer and as an admin rather than just re-run the test suite, three real gaps surfaced. First, `transactions/$transactionId.tsx` always rendered the "Dispute this transaction" form regardless of whether the transaction already had an open dispute — filling it in and submitting only failed with a 409 toast at the end, instead of the page recognizing the existing dispute upfront. Second, there was no way to see a transaction's *past* disputes at all — confirmed as a real gap, not a hypothetical, because the seed data already had a transaction (Superbalist) with two disputes across its lifetime and the UI could only ever show the most recent one. Third, and least obvious: navigating directly to another customer's dispute or transaction URL correctly 404'd at the API (no data leak — covered by `dispute-scoping.test.ts` / `transaction-scoping.test.ts`) but the *web app* rendered the generic `RouteError` crash screen ("We hit a snag") with an HTTP 500, indistinguishable from an actual bug.

Chasing the third one down a wrong turn first: the natural fix looked like making `isApiError` recognize a plain-object-shaped error, on the theory that a `createServerFn` RPC boundary crossing re-serializes a thrown `ApiError` and strips its prototype (the same *class* of bug as #52 — something not surviving a serialization boundary). That fix didn't work, and reading TanStack Start's actual RPC deserializer (`serverFnFetcher.js`) plus the live console trace showed why: the boundary doesn't re-shape a thrown `Error` into a plain object with the same fields — it round-trips *only* `.message`, dropping `status`/`code`/`errors` entirely, for any thrown value that isn't one of the two values the framework explicitly special-cases: `notFound()` and `redirect()`. No amount of shape-detection on the client side can recover fields that were never sent.

**Decision:** Transaction page: the loader now also fetches this transaction's disputes (`GET /v1/disputes?transaction_id=...`, a new optional filter added to the already-scoped, already-tested `disputesQuerySchema`/`findDisputesByUser`, reusing the same query key as the disputes list so the existing `refreshQuery` calls on submit/withdraw already invalidate it — zero new invalidation wiring). If one of them is open (`isOpenDisputeStatus`), the page shows a "You have an open dispute → View dispute" card instead of the form; the form is never rendered at all once one exists, closing the gap at its root instead of only warning about it after the fact. Any remaining (closed) disputes render as a "Dispute history" list underneath, each linking to its detail page — same data, no extra query.

Cross-user 404: `web/src/api/index.ts` gained `rejectNotFound(error)` — called from each single-entity server function's catch block (`disputeRequest`, `transactionRequest`) while the error is still the real `ApiError` instance, server-side, before anything crosses the RPC boundary. A `status === 404` throws the framework's own `notFound()` instead of letting the original error propagate; the router's existing `defaultNotFoundComponent` (`NotFound`, already wired in `router.tsx` but until now only ever reachable for a genuinely unmatched route) renders it. The response is now a real HTTP 404, and the page reads as "not found," not "broken."

**Alternatives considered:** Special-case 404s inside `RouteError` by checking `error.status === 404` — built first, reverted once the console trace showed the deserialized error has no `status` field to check on a client-side transition (it only works on the very first SSR-only load, which never goes through the lossy RPC path — an inconsistency that would silently regress the moment someone navigates to the same broken link a second time). Widening `RouteError`'s generic fallback to always trust `error.message` — rejected: it would paper over the real bug for these two known 404 cases while leaving every other loader error one accidental thrown-`Error` away from the same silent field loss; `notFound()` is the framework's actual answer to "a lookup failed," not a message-formatting problem. A second API round-trip from the transaction page to check for an open dispute before rendering the form — rejected: the disputes-by-transaction fetch already returns everything needed (open and closed) in one call.

**How it solves the problem:** Verified live as both roles — the transaction page never shows a form once a dispute is open, links straight to it, and lists prior disputes on the seeded Superbalist transaction that were previously invisible; a customer session hitting another customer's dispute or transaction URL now gets a clean 404 page instead of a crash screen, confirmed via the network tab (`404`, not `500`) and the console (the "Dispute not found." uncaught-error trace is gone). Full suite still green — 44/44 tests, `turbo lint typecheck build test` all passing — with no changes to `RouteError`/`isApiError` beyond reverting the false-start.
...
**Problem:** The four list surfaces — customer transactions, customer disputes, the admin review queue, the admin invite list — had each grown their own query schema by hand. `transactionsQuerySchema` and `disputesQuerySchema` re-declared `page`/`limit`/`order` line for line with slightly different wording; the review queue and invite list had no filtering at all beyond a bare `status`; there was a half-built `paginationSortAndSearchQuerySchema` in `shared/src/schema/global.ts` that nothing used and that typed `page`/`limit` as raw strings and `sort` as a free `string`. The frontend, meanwhile, had a working paginator but no way to search, filter by date, or sort a column from the UI. Adding those three capabilities to four pages by hand would have meant four more divergent copies.

**Decision:** `shared/src/schema/global.ts` exports `paginationQuerySchema` — `{ page, limit, search?, from?, to?, order? }` with `page`/`limit` coerced-and-bounded (`pageQuerySchema`/`limitQuerySchema` in `field.ts`), `search` trimmed and emptied-to-`undefined`, `from`/`to` as `z.iso.date()`. Every list query is now `paginationQuerySchema.extend({ sort: z.enum(XXX_SORT).optional(), …own filters })`. The `sort` whitelist is a per-endpoint SCREAMING_SNAKE constant in `shared/src/constant.ts` (`TRANSACTION_SORT`, `DISPUTE_SORT`, `ADMIN_DISPUTE_SORT = { ...DISPUTE_SORT, customer }`, `ADMIN_INVITE_SORT`) — an unlisted `?sort=` value is a `422`, never an interpolated column name. The `from ≤ to` check can't live on the base schema (a Zod object with a `.refine()` can't be `.extend()`ed further), so it's a shared predicate + issue object (`isOrderedDateRange`, `ORDERED_DATE_RANGE_ISSUE`) that each concrete query applies as its own final `.refine()`. On the API side, the string→`PgColumn` binding is a `Record<XSort, PgColumn>` map in each repository (`SORT_COLUMN` in `dispute.ts`, etc.) — it has to be there and not in `shared` because sortable columns like `merchant`/`amount_cents` live on a *joined* table, and `customer` on another; three small reusable Drizzle helpers (`containsText` with `LIKE` metacharacters escaped, `withinDays` snapping to `startOfDay`/`endOfDay` via `date-fns`, `sortDirection`) live in `api/src/database/repository/list-filters.ts`. `admin_invite.status` is not a column — it's derived (`PENDING`/`ACCEPTED`/`EXPIRED` from `accepted_at`/`expires_at`), so `?status=` there is translated to a predicate. On the web side, `ListControls` (search + date range + optional status `<select>`, applied on submit not keystroke) and `SortableHeader` (a 3-state click cycle — desc, asc, cleared — in `web/src/lib/list-query.ts`'s `nextSort`) drive the route's search params; `router.tsx` gets a custom `stringifySearch` that orders the keys (`page`/`limit` first, `sort`/`order` last) so the URL reads consistently no matter what order the UI set them in.

**Alternatives considered:** Keep `paginationSortAndSearchQuerySchema` and just fix it in place — rejected: it typed `page`/`limit` as strings and `sort` as an open `string`, so every consumer would still need its own coercion and its own column-name guard; the whole point is that the base schema hands back validated numbers and each endpoint declares a closed `sort` enum. Put the `sort`→column map in `shared` next to the whitelist — rejected: `shared` has no `PgColumn` and no knowledge of which columns are on joined tables; the whitelist (a contract) belongs in `shared`, the binding (an implementation detail of one query) belongs in the repo. A single generic `buildListQuery()` that takes a table and emits the whole `where`/`orderBy`/pagination — rejected as the exact over-abstraction #35/#36 trimmed out of the skeleton: the four queries differ in their joins, their scoping, and their searchable columns, and three shared leaf helpers (`containsText`/`withinDays`/`sortDirection`) cover the actual repetition without a DSL. Debounced search-as-you-type — rejected: apply-on-submit is one request per intent, not one per keystroke, and it keeps the URL (and the loader) from thrashing.

**How it solves the problem:** All four list endpoints now share one validated pagination/search/date base and differ only in their `sort` whitelist and their own filters; adding search + date + sort to the frontend was two shared components, not four page rewrites. Covered by `api/test/list-query.test.ts` (22 tests: coercion, the `422` on an unlisted sort, `search` across joins, the date-range refine, per-column sort direction).

## 58. Review queue: unresolved disputes float to the top by default, and the stat cards come from their own endpoint

**Problem:** The admin review queue ordered strictly by `created_at` descending — the same as the customer list. On the seeded data (~260 disputes, most already resolved) that buried every `SUBMITTED` dispute that wasn't opened in the last day or two several pages deep: the one view whose entire job is "what needs a decision" led with disputes that had already had one. There was also no at-a-glance sense of the queue's shape — how many submitted, how many in review — without paging through it.

**Decision:** Two parts. **(a) Default ordering is a work list.** With no explicit `?sort`, `findDisputesForReview` orders `status ASC, created_at ASC, id DESC` — the `disputeStatus` pgEnum is declared in lifecycle order (`SUBMITTED`, `UNDER_REVIEW`, `RESOLVED`, `REJECTED`, `WITHDRAWN`), so `status ASC` puts the two open states first, and `created_at ASC` within them means the oldest unattended dispute is at the very top, so nothing rots. Any explicit `?sort` from a column header overrides this entirely (back to the whitelisted column, `order` its direction). The customer's own dispute list keeps `created_at DESC` — a customer wants their latest, an admin wants their oldest-unresolved. **(b) The counts are a separate endpoint.** `GET /v1/admin/disputes/summary` runs one unfiltered `GROUP BY status` (`countDisputesByStatus`, zero-filled to all five statuses) and returns `{ SUBMITTED: n, … }`. The web loader `Promise.all`s it alongside the list; its query key `["admin","disputes","summary"]` is a *prefix* of the list key `["admin","disputes"]`, so the `removeQueries` inside `refreshQuery` (#52) after any review/resolve already refreshes the cards too — no extra invalidation wiring. `DisputeSummaryCards` renders one card per status; the two open statuses render solid-filled ("you need to attend to these") when their count is non-zero and quiet otherwise, and clicking a card sets/clears `?status=`.

**Alternatives considered:** Make the counts a field on the list response envelope (`count_by_status` next to `count`) — rejected: the list is filtered and paginated, the cards are neither; the counts would either have to ignore the very filters the same response applied (confusing) or change as you filter (useless as a queue overview). A materialized view or a cached counter row updated on every transition — rejected as premature: one `GROUP BY` on an indexed `status` column over a few hundred (even a few hundred thousand) rows is sub-millisecond; #48's "measure before building" applies. Encode the priority as a real `priority` column — rejected: priority here is a pure function of `status` and age, nothing a human sets; a column would be derived data that can drift from the two fields it's computed from. A separate "needs attention" tab instead of reordering the one list — rejected: it's the same rows with the same actions; ordering the single queue by urgency is less UI for the same outcome.

**How it solves the problem:** The review queue opens on the oldest dispute that still needs a decision, the cards show the queue's shape without paging, and clicking one filters to that status. `api/test/list-query.test.ts` covers the default ordering (a `SUBMITTED` dispute sorts ahead of a newer `WITHDRAWN` one; an explicit `?sort=created_at&order=desc` flips that) and the zero-fill; `api/test/admin-authz.test.ts` covers the `403` for a customer session on `/summary`.

## 59. A second cleanup pass — web-inclusive this time — plus the generic `<DataList>` the four list pages finally earned

**Problem:** #49 was a backend-only audit. Since then the frontend had grown a lot of surface fast (the whole TanStack app, #51; the list-query feature, #57; the review queue, #58), and a grep-based sweep across all three packages turned up the usual post-feature residue: dead exports left behind by refactors (`isTerminalDisputeStatus` + its constant, `formatRelative`, `disputeStatusTone`, `CardFooter`, `Label`, a handful of unused `shared` types), several things exported that only their own file used, and genuine copy-paste — four hand-rolled list-response envelopes, an `interface Page<T>` pasted into three repositories, a `DetailRow` component defined twice, the same six-line server-error-to-form mapping in six form handlers. The four list pages (`transactions`, `disputes`, `admin` queue, `admin/invites`) had also converged on an identical structure — `validateSearch` + `loaderDeps` + loader, `applyFilters`/`sortBy`, `ListControls` + `Table` + `SortableHeader`s + `Pagination` — and were now a real fourth concrete caller, not a speculative one.

**Decision:** Full audit first (findings catalogued in the session scratchpad and `docs/flows.md` was written in the same pass — 12 flows end to end, per role), then remove only what was confirmed unused. Dead code deleted; file-local exports un-`export`ed. Dedup: `api/src/lib/paginated-response.ts` `paginatedResponse({ query, total, rows, message })` returns a plain object (so the `fastify-type-provider-zod` provider still checks it against each route's response schema) and replaces the four hand-rolled envelopes; `Page<T>` moved to one file; `web/src/components/custom/detail-row.tsx` and `web/src/lib/form.ts`'s `applyServerErrors(setError, error)` replace their duplicates. `web/src/hooks/use-toast-mutation.tsx` → `web/src/lib/toast-mutation.ts`, `useToastMutation` → `runToastMutation` — it calls no hooks and runs inside event handlers, so the `use*` name was a rules-of-hooks trip hazard. **`web/src/components/custom/data-list.tsx`** — a generic `<DataList TRow TStatus TSort>` that owns `ListControls` + the sortable `Table` + `Pagination` and the apply/clear/sort-cycle/page-change handlers; each page now passes a `columns` array (`{ header, cell, sortKey?, align?, cellClassName? }`), its filter/empty copy, and `onSearchChange={(next) => navigate({ search: next })}`. `useFormField` lost its unused `type: "select"` overload and never-read `loading` return in the same pass. Two small structural calls (put to Nolan explicitly): `web/src/routes/_authenticated/admin.tsx` — a layout route whose only content was a `beforeLoad` role guard that `_authenticated.tsx` already performs — was **removed**; `API_NAMESPACE` (a const object that only fed its own `keyof typeof` type) was **inlined** to `type ApiNamespace = "AUTH" | "ADMIN" | "HEALTH" | "CUSTOMER"`.

**Alternatives considered:** Extract `<DataList>` back when the second list page appeared — rejected then and correct to have waited: the third and fourth pages revealed real divergence (a joined-field sort, a two-line customer cell, page-specific content above the list) that a two-caller abstraction would have guessed wrong. Fold `ListControls`/`SortableHeader`/`Pagination` *into* `DataList` now that it's their only caller — rejected: they're clean single-responsibility units and testable on their own; `DataList` composes them. Add `eslint-plugin-react-hooks` (the config has none, and it would have flagged the `useToastMutation` misuse) — **not done**: ESLint is on 10.4.0, the plugin's ESLint-10 support is unproven, and a broken `turbo lint` for the whole repo is a bad trade for a lint rule whose one finding is already fixed by the rename; flagged as a recommendation. Prune the ~17 unused `XxxModelUniqueWhere` / `XxxModelInsert` schema type exports — **not done** (Nolan's call): they mirror the Ubuntu Stories one-type-per-table house style, and consistency there is worth more than the dead lines.

**How it solves the problem:** The four list routes dropped ~370 lines between them for one ~150-line generic component; the duplicated envelope/`Page`/`DetailRow`/error-mapping code is each defined once; the mis-named hook can't be mistaken for one. `turbo typecheck lint` green across all three packages, `api` 63 tests, `web` 7 (the `disputeStatusTone` test went with the dead code). Every list surface — search, status filter, 3-state column sort, pagination, empty state, the review queue's card-filter composing with a column sort — verified live in Chrome as both roles; the one console warning is the pre-existing `formatDate` SSR/timezone mismatch (#56's deferred list), unchanged by this work.
