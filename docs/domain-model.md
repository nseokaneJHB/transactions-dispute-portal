...
- **Dispute** — belongs to a transaction; status: `SUBMITTED → UNDER_REVIEW → RESOLVED | REJECTED`, plus a customer-only `WITHDRAWN` exit from either open state; reason: `FRAUDULENT_CHARGE | DUPLICATE_CHARGE | INCORRECT_AMOUNT | GOODS_NOT_RECEIVED | SUBSCRIPTION_NOT_CANCELLED | OTHER`. Enum values are SCREAMING_SNAKE end to end (`docs/decisions.md` #30). A DB-level partial unique index enforces at most one `SUBMITTED`/`UNDER_REVIEW` dispute per transaction (`docs/decisions.md` #4/#28/#40). The lifecycle is forward-only — `/review` moves `SUBMITTED → UNDER_REVIEW`, `/resolve` requires `UNDER_REVIEW`, `/withdraw` closes an open dispute as `WITHDRAWN`, terminal states are terminal, each transition guarded in the `UPDATE … WHERE` (`docs/decisions.md` #41/#45). "Reopen after a rejection" is opening a *new* dispute (the index only blocks a second *open* one); admin-declines-review and supervisor-override are out of scope (#45).
...
- **AuthAuditLog** — one row per auth event (OTP requested, login success/failure, OTP lockout, email-change requested); keyed by `email` since a failed attempt may not resolve to a user (`docs/decisions.md` #28/#46)
